import { useState, useRef, useEffect } from 'react';
import { setSort } from '../../redux/actions/filter';
import { useDispatch, useSelector } from 'react-redux';
import type { RootState } from '../../redux/store';
import { SortOption } from '../../types/filter';

const sortItems: SortOption[] =  [
    { name: 'популярности', type: 'popular', order: 'desc' },
    { name: 'цене', type: 'price', order: 'asc' },
    { name: 'алфавиту', type: 'name', order: 'asc' },
];

const Sort = () => {
    const dispatch = useDispatch();
    const currentSort = useSelector((state: RootState) => state.filters.sortBy);
    const sortRef = useRef<HTMLDivElement>(null);
    const [activeSort, setActiveSort] = useState(0);
    const [sortPopup, setSortPopup] = useState(false);

    const selectOnClick = (index: number) => {
        setSortPopup(false);
        setActiveSort(index);
        dispatch(setSort(sortItems[index]));
    };

    const handleClicksOutSide = (event: any) => {
        if (!event.path.includes(sortRef.current)) {
            setSortPopup(false);
        }
    };

    useEffect(() => {
        document.body.addEventListener('click', handleClicksOutSide);
    }, []);
    return (
        <div ref={sortRef} className='sort'>
            <div className='sort__label'>
                <svg
                    className={sortPopup ? 'active' : ''}
                    width='10'
                    height='6'
                    viewBox='0 0 10 6'
                    fill='none'
                    xmlns='http://www.w3.org/2000/svg'>
                    <path
                        d='M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z'
                        fill='#2C2C2C'
                    />
                </svg>
                <b>Сортировка по:</b>
                <span onClick={() => setSortPopup(!sortPopup)}>
                    {sortItems[activeSort].name}
                </span>
            </div>
            {sortPopup && (
                <div className='sort__popup'>
                    <ul>
                        {sortItems.map((item, index) => {
                            return (
                                <li
                                    key={item.name}
                                    onClick={() => selectOnClick(index)}
                                    className={index === activeSort ? 'active' : ''}>
                                    {item.name}
                                </li>
                            );
                        })}
                    </ul>
                </div>
            )}
        </div>
    );
}

export default Sort;